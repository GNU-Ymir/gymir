...
namespace semantic {
	...
	generator::Generator CallVisitor::validateTemplateClassCst (const lexing::Word &, const Generator & ref, const std::vector <generator::Generator> & rights_, int & score, std::vector <std::string> & errors, Symbol & _sym, Generator & proto_gen) {
	    std::vector <Generator> typeParams;
	    std::vector <Generator> valueParams;
	    std::vector <Generator> rights = rights_;

	    for (auto & it : ref.to <TemplateClassCst> ().getConstructor ().to <syntax::Constructor> ().getPrototype ().getParameters ()) {
		Generator value (Generator::empty ());
		auto var = it.to<syntax::VarDecl> ();
		volatile bool failure = false;
		this-> _context.enterForeign ();		    // We enter a foreign, the value of a parameter var has no local context
