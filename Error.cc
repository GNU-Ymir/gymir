...
#define MAX_ERROR 50
	...
	if (isActive) __isEnable__.pop_back ();
	else __isEnable__.push_back (isActive);
	if (isActive) {
	    __caught__.clear ();
	}
    }
    ...
    void Error::returnLocalAddr (const Word & word, const Word &word2) {
	std::string msg = format (getString (AddrLocalVar), YELLOW, word.getStr (), RESET);
	msg = std::string (RED) + "Error" + std::string (RESET) + " : " + msg;
	msg = addLine (msg, word);
    ...
    void Error::dynamicAccess (const Word& word) {
	std::string msg = getString (DynamicAccess);
	msg = std::string (RED) + "Error" + std::string (RESET) + " : " + msg;
	msg = addLine (msg, word);
	ErrorMsg errorMsg = {msg, false, false};
	if (__isEnable__.back ()) {
    ...
    void Error::allocationInSafe (const Word& word) {
	std::string msg = getString (AllocationInSafe);
	msg = std::string (RED) + "Error" + std::string (RESET) + " : " + msg;
	msg = addLine (msg, word);
	ErrorMsg errorMsg = {msg, false, false};
	if (__isEnable__.back ()) {
    ...
    void Error::callUnsafeInSafe (const Word& word) {
	std::string msg = getString (CallUnsafeInSafe);
	msg = std::string (RED) + "Error" + std::string (RESET) + " : " + msg;
	msg = addLine (msg, word);
	ErrorMsg errorMsg = {msg, false, false};
	if (__isEnable__.back ()) {
	...
	std::string msg = format (line, YELLOW, size, RESET, YELLOW, index, RESET);
	msg = std::string (RED) + "Error" + std::string (RESET) + " : " + msg;
	msg = addLine (msg, word);
	ErrorMsg errorMsg = {msg, false, false};
	if (__isEnable__.back ()) {
	...
	auto msg = RED + std::string ("Error") + RESET + std::string (" : ") + format (str, YELLOW, word.getStr ().c_str (), RESET);
	...
	auto msg = RED + std::string ("Error") + RESET + std::string (" : ") + format (str, expected, YELLOW, word.getStr ().c_str (), RESET);
    ...
    void Error::syntaxErrorFor (const Word & word) {
	if (__isEnable__.back ()) {
	    Error::note (word, "");
	    THROW;
	} 
	...
	auto msg = RED + std::string ("Error") + RESET + std::string (" : ") + std::string (getString (EscapeChar));	
	msg = addLine (msg, word);
	ErrorMsg errorMsg = {msg, false, false};
	if (__isEnable__.back ()) {
	    Error::instance ().nb_errors ++;
    ...
    void Error::endOfFile (const Word& word) {
    ...
    void Error::templateCreation (const Word& word, std::string values) {	
	auto str = getString (TemplateCreationWith);
	auto msg = format (str, values);
	msg = std::string (BLUE) + "Note" + std::string (RESET) + " : " + std::string (msg);
	msg = addLine (msg, word);
	ErrorMsg errorMsg = {msg, false, false};
	if (__isEnable__.back ()) {
    ...
    void Error::useAsTrait (const Word& word) {
	auto str = getString (UseAsTrait);
	auto msg = format (str, YELLOW, word.getStr ().c_str (), RESET);	
	msg = std::string (RED) + "Error" + std::string (RESET) + " : " + std::string (msg);
	msg = addLine (msg, word);
	ErrorMsg errorMsg = {msg, false, false};
    ...
    void Error::staticNoInit (const Word& word) {
	auto str = getString (StaticNoInit);
	auto msg = format (str, YELLOW, word.getStr ().c_str (), RESET);
	msg = std::string (RED) + "Error" + std::string (RESET) + " : " + std::string (msg);
	msg = addLine (msg, word);
	ErrorMsg erroMsg = {msg, false, false};
    ...
    void Error::multipleDestr (const Word & token) {
	std::string msg = format (getString (MultiDestr));
	msg = std::string (RED) + "Error" + std::string (RESET) + " : " + std::string (msg);
	msg = addLine (msg, token);
    ...
    void Error::staticMethodInit (const Word & token) {
	std::string msg = format (getString (StaticMethodInit), YELLOW, RESET, YELLOW, RESET);
	msg = std::string (RED) + "Error" + std::string (RESET) + " : " + std::string (msg);
	msg = addLine (msg, token);
    ...
    void Error::variadicMustBeLast (const Word & token) {
	std::string msg = format (getString (VariadicMustBeLast), YELLOW, token.getStr (), RESET); 
	msg = std::string (RED) + "Error" + std::string (RESET) + " : " + std::string (msg);
	msg = addLine (msg, token);
