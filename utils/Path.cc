...
namespace Ymir {
    ...
    Path Path::getCwd () {
        ...
        return Path (std::vector <std::string> ());
    }
    ...
    Path Path::toAbsolute (const Path & cwd) const {
        if (this-> _files.size () == 0) return cwd;
        else {
            if (this-> _files [0] != "") {
                return Path::build (cwd, this-> toString ("/")).simplify ();
            } else {
    ...
    Path Path::simplify () const {
        ...
        for (uint64_t i = 0 ; i < this-> _files.size () ; i++) {
            if (this-> _files [i] == ".." && results.size () != 0) {
                results.pop_back ();
            } else if (this-> _files [i] != "."){
                results.push_back (this-> _files [i]);
            }
        }
