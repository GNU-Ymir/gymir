...
namespace generic {
	 ...
	 */
	static Tree intType (int32_t size, bool isSigned);
	 ...
	 * \param right the right operand
	 */
	static Tree binaryPtrTest (const lexing::Word & location, tree_code code, const Tree & type, const Tree & left, const Tree & right);
	 ...
	 */
	static Tree constructIndexed0 (const lexing::Word & loc, const Tree & type, const Tree & value, uint32_t size);
	 ...
	 */
	static Tree buildPtrCst (const lexing::Word & loc, uint64_t value);	
	
	/**
	 * \brief Create a uint cst
	 */
	static Tree buildIntCst (const lexing::Word & loc, uint64_t value, const Tree & type);
	
	/**
	 * \brief create a int cst
	 */
	static Tree buildIntCst (const lexing::Word & loc, int64_t value, const Tree & type);

	/**
	 * \brief create a string literal
	 */
	static Tree buildStringLiteral (const lexing::Word & loc, const char * content, uint64_t size, int32_t innerSize);
	
	/**
	 * \brief create a size cst
	 */
	static Tree buildSizeCst (uint64_t size);
	 ...
	 * \brief Generate a bool const
	 */
	static Tree buildCharCst (const lexing::Word & loc, uint32_t value, const Tree & type);
	 ...
	 */
	Tree getStringSize (int32_t inner) const;
	 ...
	 */
	Tree getOperand (int32_t i) const;
	 ...
	 */
	Tree buildPointerUnref (int32_t index) const;

	/**
	 * \return create a pointer unref at index 
	 * \param type the type that is generated by unref (in case of *(&void))
	 */
	Tree buildPointerUnref (Tree type, int32_t index) const;
