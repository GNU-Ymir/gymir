...
namespace Ymir {    
    DECLARE_ENUM_WITH_TYPE (ExternalError, std::string,
			    ...
			    CONDITIONAL_NON_TEMPLATE_CLASS  = "conditional block in a non template class",
			    CONDITIONAL_NON_TEMPLATE_TRAIT  = "conditional block in a non template trait",
			    ...
			    DECL_VARIADIC_FUNC              = "C variadic functions can only be external",
			    ...
			    DOC_FILE_ERROR                  = "writing doc file : '%(y)'",
			    ...
			    EXTERNAL_VAR_DECL_WITHOUT_TYPE	    = "global var %(y) declared external must have a type",
			    ...
			    GLOBAL_VAR_DECL_WITHOUT_VALUE   = "global var %(y) declared without value",
			    GLOBAL_VAR_DEPENDENCY           = "the global var %(y) cannot be initialized from the value of %(y)",
			    ...
			    MONITOR_NON_CLASS               = "cannot synchronized unmonitored type %(y)",
			    ...
			    MULTIPLE_DESTRUCTOR             = "class can have only one destructor",
			    ...
			    NOT_CTE_ITERABLE                = "the type %(y) is not iterable at compile time",
			    NOT_IMPL_TRAIT                  = "the type %(y) does not implement trait %(y)",
			    ...
			    OF                              = "of : %",
			    OTHER_ERRORS                    = "there are other errors, use option -v to show them",
			    ...
			    PROTECTION_NO_IMPACT            = "protection %(y) has no impact on %(y) declaration",
			    ...
			    RETHROW_NOT_MATCHING_PARENT     = "rethrowing an exception %(y) that is not rethrowed by parent method is not allowed", 
			    ...
			    TYPE_NO_FIELD                   = "type %(y) has no fields",
			    ...
			    UNDEFINED_CALL_OP		    = "the call operator is not defined for %(y) and {%(y)}",
			    ...
			    UNKNOWN_LAMBDA_TYPE             = "lambda function is not complete, missing type for parameter %(y)",
			    ...
			    VAR_DECL_IN_TRAIT               = "field declaration inside trait is forbidden",
